...
class BlogController extends Controller {
     ...
     */
    async create() {
        const { title, content, htmlContent, tags, firstImg, weather, category } = this.ctx.request.body;
        const uid = this.ctx.session.uid || '1';
        const summary = content; // TODO
        const insertResult = await this.service.blog.insert(uid, title, content, summary, tags, firstImg, weather, htmlContent, category);
        this.ctx.body = {
